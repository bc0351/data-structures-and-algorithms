...
function transformToLis(obj){
  let arr =[];
  for(let k in obj) { arr.push(`<li>${k}: ${obj[k]}</li>`)};
  return arr;
};
...
const count = (target, input) => {
  return [].concat.apply([], input).filter(e => e === target).filter(e => e).length;
};
...
const totalSum = (input) => {
  return [].concat.apply([], input).reduce((a,b) => {return a + b});
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => {return arr.filter(e => typeof e === 'number' && e % 5 === 0).map(e => {return Math.pow(2,e)})});
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
let findMaleAndFemale = (data) => {
  return data.map(e => {return (e['gender'] === 'female' || e['gender'] === 'male') ? e.name : null}).filter(e => e).join(' and ');
};
...
let findShortest = (data) => {
  return data.map(o => {return [parseInt(o.height), o.name]}).sort((a, b) => a[0] - b[0])[0][1];
};
