...
const validateEmail = (email) => {
  let regexp = /^([a-zA-Z0-9]+|[a-zA-Z0-9]+\.[a-zA-Z0-9]+)@[a-zA-Z0-9]+\.(net|com|org)$/;
...
const validatePhoneNumber = (phoneNumber) => {
  let regexp = /^(\([0-9]{3}\)\s[0-9]{3}\-[0-9]{4}|\([0-9]{3}\)[0-9]{3}(\s|\-)[0-9]{4}|[0-9]{3}(\s|\-)[0-9]{3}(\s|\-)[0-9]{4}|[0-9]{3}(\s|[0-9]{1}|\-)([0-9]{3}|(\s|\-))[0-9]*|[0-9]{6}-[0-9]{4})$/gm;
  return console.log(`${phoneNumber} ${regexp.test(phoneNumber)}`);
};
...
const findTagNames = elements => {
  let regexp = /(\<\/[a-zA-Z0-9]+\>)/gm;
  let regexpReplace = /[\<\>]/g;
  return elements.map(e => {return e.match(regexp)}).flat(1).map(e => {let tag = e.replace(regexpReplace, ''); return tag});

  // return elements.map(e => {return (typeof e === 'array')
  //   ? e.map(eElem => {return eElem.match(regexp).replace(/(\<|\>)/m,'')})
  //     : (typeof e === 'object') ? e.map(o => {return o[Object.keys(o)].match(regexp).replace(/(\<|\>)/m,'')}) 
  //   : e.match(regexp)}).flat(1);
};
