...
const sortByChildren = (charArray) => {
  return charArray.sort((charA, charB) => {return (charB.children.length === charA.children.length)?(charA.name < charB.name)?-1:1:(charB.children.length > charA.children.length)?-1:1;});
};
...
const isCapitalized = (str) => {
  let regex = /^([A-Z][a-z]+)/;
  return str.split(' ').map(e => e.match(regex)).filter(e => e).map(e => [...new Set(e)][0]);
};
...
const citiesAtoJ = (arr) => {
  let regex = /^([A-J][A-Za-z\s]+)$/;
  return arr.map(str => str.match(regex)).filter(e => e).map(e => [...new Set(e)][0]);
};
