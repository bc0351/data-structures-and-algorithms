...
const sortStarWarsCharacters = (starWarsArr) => {
  return starWarsArr.sort((a, b) => {return parseInt(b.height) > parseInt(a.height)?  1 : -1;});
};
...
const removeThree = (idx, arr) => {
  return arr.filter((element, index) => { return (idx <= index && index <= idx + 2) ? null : element});
};
...
const listFoods = (recipe) => {
  return(recipe.ingredients.map(e=>{return e.replace(/^\d+\s(pound|pounds|cup|cups|gallon|gallons|medium\-sized)\s/i,'')}));
};
