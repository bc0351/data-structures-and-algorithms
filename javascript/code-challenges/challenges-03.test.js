...
const typeNum = (arr) => {
  return arr.filter(e=>(!isNaN(e))?e:null);
};
...
const containsAnd = (arr) => {
  return arr.filter(e=>e.includes('and')?e:null);
};
...
const oddValues = (arr) => {
  return arr.filter(e=>(e%2===0)?null:e);
};
...
const getStatName = (arr, minBaseStat) => {
  return arr.filter(e=>(e.baseStat>minBaseStat)).map(e=>e.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(e=>(e.children)?null:e.name);
};
...
const evenOddNumericValues = (arr) => {
  return arr.filter(e=>typeof e === "number").map(e => ( e % 2 === 0 ) ? 'even' : 'odd' ); 
};
