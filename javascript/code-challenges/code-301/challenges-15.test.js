...
const screenForNames = (arr) => {
  let regexp = /^(((Mr|Mrs|Ms|Dr)\.)\s[a-zA-Z]+)$/igm;
  return arr.map(str => {return regexp.test(str) ? str : null}).flat(1).filter(e => e);
}
...
const toTitleCase = (arr) => {
  return arr.map(str => {return str.split('').map((c,i) => {return (i === 0) ? c.toUpperCase() : c}).join('')});
};
...
let biggerThanLuke = (arr) => {
  let lukeObj = arr.filter(obj => (obj.name === 'Luke Skywalker'));
  return arr.filter(obj => parseInt(obj.mass) > parseInt(lukeObj[0].mass)).map(o => o.name).join(' - ');
};
...
const sortBy = (property, arr) => {
  return arr.sort((a,b) => {return a[property] < b[property] ? -1 : 1});
};
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  let regexp = /^(https\:\/\/)[a-zA-Z0-9]+\.[a-z]{3}$/;
...
const detectTicTacToeWin = (board) => {
  if (!helpCheck(board, 0,0,0,1,0,2)) {
    if (!helpCheck(board, 1,0,1,1,1,2)) {
      if (!helpCheck(board, 2,0,2,1,2,2)) {
        if (!helpCheck(board, 0,0,1,0,2,0)) {
          if (!helpCheck(board, 0,1,1,1,2,1)) {
            if (!helpCheck(board, 0,2,1,2,2,2)) {
