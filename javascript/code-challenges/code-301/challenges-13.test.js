...
const longestString = (arr) => {
  ...
  return arr.indexOf(arr.filter(str=> {return (str.length === maxLen) ? str : undefined})[0]);
};
...
const firstLetters = (arr) => {
  let regexp = /[a-zA-Z]{1}/;
  return arr.map(str => {return str.match(regexp)}).flat(1);
};
...
const findHappiness = (arr) => {
 let regexp = /(\:\))/;
 return arr.map(str => {return (regexp.test(str)) ? str : null}).filter(e => e);
};
...
const standardizePhoneNumbers = (arr) => {
  let regexp = /[^\d]/gm;
  return arr.map(str => {return str.replace(regexp,'')});
};
...
const onlyOddChars = (str) => {
  let regexp = /^\d*[13579]$/;
  return str.split('').map((char, index) => {return (regexp.test(index.toString()) ? char : null)}).filter(e => e).join('');
};
...
const allHappy = (arr) => {
  let regexp = /^.*(\:\)).*$/;
  let tfArr = arr.map(str=> {return regexp.test(str)});
  let result = true;
  for (let v of tfArr) {if (v === false) { result = false }};
  return result;
};
